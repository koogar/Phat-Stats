...
void Display_LS_RETRO () {
  ...
  if (stringComplete) {
...
#ifdef Debug
    tft.setTextColor(ILI9341_YELLOW, ILI9341_RED); // used to stop flickering when updating digits that do not increase in length. CPU/GPU load still need a clear box on the end digits
#endif
    ...
    tft.drawRoundRect  (106, 90, 88, 22, 4, ILI9341_YELLOW); //

    /* GPU Portrait BOOST/TJMAX, */
    tft.fillRoundRect  (107, 210, 86, 20, 4, ILI9341_BLACK);   //
    tft.drawRoundRect  (106, 209, 88, 22, 4, ILI9341_YELLOW); //

    /* CPU  Freq Line */
    tft.drawFastHLine(110, 50, 200,  ILI9341_YELLOW);

    /*GPU Memory Used Line */
    tft.drawFastHLine(110, 170, 200, ILI9341_YELLOW);
    ...
    /* CPU Outline, */
    tft.drawRoundRect  (13,  22, 88,  89, 6,    ILI9341_YELLOW);

    /* GPU Outline, */
    tft.drawRoundRect  (13, 144, 88,  89, 6,    ILI9341_YELLOW);

    tft.drawRoundRect  (0,   0  , 320, 120, 8,    ILI9341_YELLOW);

    tft.drawRoundRect  (0, 124, 320, 116, 8,    ILI9341_YELLOW);


    //------------------------------------CPU/GPU/RAM BMP IMAGES--------------------------------------------

    /* Blank CPU PCB BMP, */
    tft.drawBitmap(16, 25, CPU3_BMP, 82, 82, ILI9341_YELLOW);

#ifdef INTEL_CPU
    //tft.drawBitmap(16, 25, IntelCoreOnly_BMP, 88, 82, ILI9341_YELLOW);
    tft.drawBitmap(16, 25, IntelCoreOnly_BMP, 88, 82, ILI9341_YELLOW);
#endif

#ifdef AMD_CPU
    tft.drawBitmap(16, 25, AMDCoreOnly_BMP, 88, 82, ILI9341_YELLOW);
#endif
    ...
    tft.drawBitmap(16, 148, Nvidia_Logo_BMP, 82, 82, ILI9341_YELLOW); // Nvidia Logo
#endif

#ifdef NVIDIA_GTX_Ti_GRAPHICS
    tft.drawBitmap(16, 148, NvidiaGTX_Ti_Logo_BMP, 82, 82, ILI9341_YELLOW); // Nvidia Logo
#endif

#ifdef NVIDIA_RTX_GRAPHICS
    tft.drawBitmap(16, 148, NvidiaRTX_Logo_BMP, 82, 82, ILI9341_YELLOW); // Nvidia Logo
#endif

#ifdef NVIDIA_RTX_SUPER_GRAPHICS
    tft.drawBitmap(16, 148, NvidiaRTXSuper_Logo_BMP, 82, 82, ILI9341_YELLOW); // Nvidia Logo
#endif

#ifdef AMD_GRAPHICS
    tft.drawBitmap(16, 148, RADEON_Logo_BMP, 82, 82, ILI9341_YELLOW); // Nvidia Logo
#endif

#ifdef INTEL_GRAPHICS
    //tft.fillRoundRect  (14, 141, 86,  87, 5,    ILI9341_YELLOW);  // INTEL GPU Logo
    tft.drawBitmap(13, 147, IntelCoreOnly_BMP, 88, 82, ILI9341_YELLOW);
#endif
    ...
    tft.setCursor(284, 9);
    tft.print("RX");
    tft.fillCircle(306, 12, 7, ILI9341_YELLOW);// Landscape Flash RX top right corner when updating
    tft.drawCircle(306, 12, 8, ILI9341_YELLOW);
...
#define ILI9341_updateBox  ILI9341_BLACK // Default for normal running
    ...
    */
    ...
    tft.drawFastHLine(220, 75, 71, ILI9341_YELLOW);
    tft.setTextSize(1);
    ...
    /* Network Outline, */
    //                 ( X  , Y ,  W , H , Radius ,    Color
    tft.drawRoundRect  (196, 209, 120, 22, 2, ILI9341_YELLOW); //
    ...
    CustomTriggerCPU_BOOST_LS_RETRO( cpuClockString.toInt     ()); // CPU Frequency
    CustomTriggerGPU_BOOST_LS_RETRO( gpuCoreClockString.toInt ()); // GPU Frequency
#endif

#ifdef enable_ThrottleIndicator
    CustomTriggerCPU_ThrottleIndicator_LS_RETRO( cpuString1.toInt() ); //  CPU TJMax/Throttle Incicator BMP
    CustomTriggerGPU_ThrottleIndicator_LS_RETRO( gpuString1.toInt() ); //  GPU TJMax/Throttle Incicator BMP
#endif
...
void CustomTriggerCPU_ThrottleIndicator_LS_RETRO(int cpuDegree ) {  // i5-9600k TJMax is 100c
  float CPUtempfactor = cpuDegree ;

  if (CPUtempfactor >= CPU_TJMAX ) {  // TJ Max for the Intel 9900K 100c

    /* CPU Junction Max Throttle Temp, */
    tft.fillRoundRect  (107, 91, 86, 20, 4, ILI9341_YELLOW);   //
    tft.setTextSize(2);
...
void CustomTriggerGPU_ThrottleIndicator_LS_RETRO(int gpuDegree ) {
  float GPUtempfactor = gpuDegree ;

  if (GPUtempfactor >= GPU_TJMAX ) {  //GTX 1080 TJMax = 83c

    /* GPU Junction Max Throttle Temp, */
    tft.fillRoundRect  (107, 210, 86, 20, 4, ILI9341_YELLOW);   //
...
void CustomTriggerCPU_BOOST_LS_RETRO(int cpuClockString ) {
...
void CustomTriggerGPU_BOOST_LS_RETRO(int gpuCoreClockString ) {
